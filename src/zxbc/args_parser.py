...
def parser() -> argparse.ArgumentParser:
    ...
    parser_.add_argument('-d', '--debug', dest='debug', action='count',
                         help='Enable verbosity/debugging output. Additional -d increase verbosity/debug level')
    parser_.add_argument('-O', '--optimize', type=int,
                         help='Sets optimization level. '
                              f'0 = None (default level is {OPTIONS.optimization_level}')
    parser_.add_argument('-o', '--output', type=str, dest='output_file',
    ...
    output_file_type_group.add_argument('-T', '--tzx', action='store_true',
                                        help="Sets output format to tzx (default is .bin)")
    output_file_type_group.add_argument('-t', '--tap', action='store_true',
                                        help="Sets output format to tap (default is .bin)")
    output_file_type_group.add_argument('-A', '--asm', action='store_true',
    ...
    parser_.add_argument('-S', '--org', type=str,
                         help=f"Start of machine code. By default {OPTIONS.org}")
    parser_.add_argument('-e', '--errmsg', type=str, dest='stderr',
                         help='Error messages file (standard error console by default)')
    parser_.add_argument('--array-base', type=int,
                         help=f'Default lower index for arrays ({OPTIONS.array_base} by default)')
    parser_.add_argument('--string-base', type=int,
                         help=f'Default lower index for strings ({OPTIONS.array_base} by default)')
    parser_.add_argument('-Z', '--sinclair', action='store_true',
                         help='Enable by default some more original ZX Spectrum Sinclair BASIC features:'
                              ' ATTR, SCREEN$, POINT')
    parser_.add_argument('-H', '--heap-size', type=int,
                         help=f'Sets heap size in bytes (default {OPTIONS.heap_size} bytes)')
    parser_.add_argument('--debug-memory', action='store_true', default=None,
                         help='Enables out-of-memory debug')
    parser_.add_argument('--debug-array', action='store_true', default=None,
                         help='Enables array boundary checking')
    parser_.add_argument('--strict-bool', action='store_true', default=None,
    ...
    case_sens_group.add_argument('-i', '--ignore-case', action='store_true', default=None, dest='ignore_case',
                                 help='Ignore case. Makes variable names are case insensitive')
    case_sens_group.add_argument('+i', '--case-sensitive', action='store_false', default=None, dest='ignore_case',
                                 help='Ignore case. Makes variable names are case insensitive')
    ...
    parser_.add_argument('--strict', action='store_true', default=None,
                         help='Enables strict mode. Force explicit type declaration')
    parser_.add_argument('--headerless', action='store_true', default=None,
    ...
    parser_.add_argument('--append-headless-binary', default=[], action='append',
                         help='Appends binary to tape file (only works with -t or -T)')
    parser_.add_argument('-N', '--zxnext', action='store_true', default=None,
    ...
    parser_.add_argument('--expect-warnings', type=int,
    ...
    parser_.add_argument('+W', '--enable-warning', type=parse_warning_option, action='append',
                         help='Disables warning WXXX (i.e. -W100 disables warning with code W100)')
    parser_.add_argument('--hide-warning-codes', action='store_true', default=None,
