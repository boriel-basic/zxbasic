...
'
' Override zx81sd: la version zx48k calcula direcciones de pantalla y
' atributos con constantes fijas de la ROM del Spectrum ($4000/$5800,
' aqui "64"/"88" como bytes altos) y con sysvars fijos de la ROM
' ($5C7B = UDG, 23693 = ATTR_P, 15360 = CHARS-256). En zx81sd:
'   - UDG/ATTR_P/CHARS son sysvars propios en $8000+ (ver sysvars.asm,
'     namespace core, de ahi el prefijo .core. en las referencias de
'     mas abajo), y CHARS/UDG son VARIABLES (punteros), no constantes,
'     asi que se leen con LD (r16),(CHARS) en vez de un LD inmediato.
'   - SCREEN_ADDR/SCREEN_ATTR_ADDR tambien son variables (no EQU fijos:
'     bloque 6 en $C000/$D800 en vez de $4000/$5800), pero el byte alto
'     se sigue sumando dentro de un ADD A,n con constante inmediata (el
'     algoritmo de "tercios" de pantalla es identico al del Spectrum,
'     solo cambia la base). Se resuelve parcheando esa constante una
'     vez al entrar en print42(), leyendola de SCREEN_ADDR+1/
'     SCREEN_ATTR_ADDR+1 (automodificacion de codigo, igual que hacia
'     la ROM original con sus propias rutinas).
' --------------------------------------------------
...
asm
    ...
    ; referencias a CHARS/UDG/ATTR_P/SCREEN_ADDR/SCREEN_ATTR_ADDR de mas
    ; abajo confian en que sysvars.asm ya este incluido por el resto del
    ...
    ; abajo (ver __P42_SCR_HI/__P42_ATTR_HI) con los bytes altos reales
    ; de .core.SCREEN_ADDR/.core.SCREEN_ATTR_ADDR de este programa (variables en
    ; zx81sd, no EQU fijos como en el Spectrum).
    LOCAL __P42_SCR_HI, __P42_ATTR_HI
    ld a, (.core.SCREEN_ADDR+1)
    ld (__P42_SCR_HI+1), a
    ld a, (.core.SCREEN_ATTR_ADDR+1)
    ld (__P42_ATTR_HI+1), a
...
printudg
      ...
      ld hl, .core.UDG       ; examine our own .core.UDG sysvar for the udg address
...
LOCAL calcChar
calcChar: ; this is the calculate-from-charset option
      ; a holds the column kill data
      ld de, (.core.CHARS)   ; Character set-256 (variable en zx81sd, no 15360 fijo)
      ld l, c          ; Get our character back from C
      call mult8       ; Multiply l by 8 and add to DE. (HL points at the charset data for our character now)
...
__P42_ATTR_HI:
      add a, 88       ; Add in start of screen attributes high byte (parcheado con .core.SCREEN_ATTR_ADDR+1 al entrar en print42)
      ...
      ld a, (.core.ATTR_P)   ; ATTR P     Permanent current colours, etc (as set up by colour statements).
      ...
      ld (hl), e       ; Must be yes - we're setting the attributes in the next square too.
...
__P42_SCR_HI:
      add a, 64       ; Turn it into screen bitmap high byte (parcheado con .core.SCREEN_ADDR+1 al entrar en print42)
