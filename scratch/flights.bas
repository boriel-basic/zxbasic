...
1040 LET ad=ad+rl: IF ad<0 THEN LET ad=ad+360
...
1520 RETURN 
2180 IF gc<>0 THEN GO SUB 2200: END IF
2190 LET as0=as0+16*(tc*30-as0-8*pt)/as0: GO SUB 2200: GO TO 2205
2200 PLOT 35,50: DRAW OVER 1;15*SIN (as0*PI/200),15*COS (as0*PI/200): RETURN 
2205 IF gc<>0 THEN PLOT 155,50: DRAW OVER 1;10*SIN (tn*PI/5),10*COS (tn*PI/5): PLOT 155,50: DRAW OVER 1;15*SIN (un*PI/500),15*COS (un*PI/500): END IF
2210 LET tn=pz/1000: LET un=pz-1000*INT tn: PLOT 155,50: DRAW OVER 1;10*SIN (tn*PI/5),10*COS (tn*PI/5): PLOT 155,50: DRAW OVER 1;15*SIN (un*PI/500),15*COS (un*PI/500)
...
2230 PLOT 215,50: DRAW OVER 1;15*SIN (tc*PI/5),15*COS (tc*PI/5): RETURN 
...
2295 IF gc<>0 THEN PLOT OVER 1;x1,168-y1: DRAW OVER 1;x2-PEEK 23677,168-y2-PEEK 23678: END IF
...
2370 PLOT OVER 1;x1,168-y1: DRAW OVER 1;x2-PEEK 23677,168-y2-PEEK 23678
2376 IF (rl=rr AND pp=pt) THEN GO TO 2500: END IF
2377 IF (y3<=2 AND y4<=2) OR (y3>=90 AND y4>=90) THEN GO TO 2380: END IF
2378 IF gc<>0 THEN PLOT OVER 1;x3,176-y3: DRAW OVER 1;x4-PEEK 23677,(176-y4)-PEEK 23678: END IF
...
2445 OVER 1: PLOT x3,176-y3: DRAW x4-PEEK 23677,(176-y4)-PEEK 23678: OVER 0
2500 GO SUB 8000
...
5110 PLOT 10,175: DRAW 235,0: DRAW 0,-90: DRAW -235,0: DRAW 0,90
5120 FOR kk=0 TO 3: CIRCLE 35+kk*60,50,20: NEXT kk
5130 PRINT AT 12,2;"SPEED   HORZN   ALT    RPM"
5150 PRINT AT 20,0;"BEARING  RUNWAY  DRIFT  DISTANCE"
5170 PLOT 87,50: DRAW 5,0: DRAW 3,-3: DRAW 3,3: DRAW 5,0
...
6030 FOR n=0 TO 20 STEP .5: PLOT 127,130: DRAW 120-INT (RND*240),45-INT (RND*90): BEEP .005,20-n: NEXT n
6040 PAUSE 50
...
6150 LET a$=INKEY$: IF a$<>"Y" AND a$<>"N" THEN GO TO 6150
6160 IF a$="N" THEN GO TO 6900: END IF
6170 CLS : GO TO 1
6900 STOP 
7000 FOR kk=0 TO 2*PI STEP PI/5: PLOT x+17*SIN kk,y+17*COS kk: DRAW 2*SIN kk,2*COS kk: NEXT kk: RETURN 
8000 IF gc<>0 THEN PLOT 127,174: DRAW OVER 1;ox,oy: END IF
8010 LET ox=16*SIN (rb*(PI/180)): LET oy=-(16*ABS COS (rb*(PI/180)))
8020 PLOT 127,174: DRAW OVER 1;ox,oy
8025 LET wb=ad: IF ad>180 THEN LET wb=wb-360
8026 IF rb>180 THEN LET wb=wb+360-rb: GO TO 8040: END IF
8030 LET wb=wb-rb
8040 IF rw=1 THEN PLOT OVER 1;rdx,175-rdy: END IF
...
8090 LET rw=1: PLOT OVER 1;rdx,175-rdy
8100 RETURN 
