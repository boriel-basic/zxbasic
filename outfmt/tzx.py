...
class TZX(object):
	...
	def save_header(self, type, title, length, param1, param2):
		...
		bytes = [BLOCK_TYPE_HEADER, type] + title_bytes + self.LH(length) + self.LH(param1) + self.LH(param2)
	...
	def save_code(self, title, addr, bytes):
		...
		bytes = [BLOCK_TYPE_DATA] + [(int(x) & 0xFF) for x in bytes] # & 0xFF truncates to bytes
		self.standard_block(bytes)
